...
#define CONTENT_IDENTIFIER @"HomeViewController"
...
- (void)viewDidLoad {
    ...
    [appSettings setValue:@"Main" forKey:@"currentViewController"]; //31-May-2017_PS addition for checking which fetch method to call in Appdelegate
    ...
    //--+--//
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(loadNewsCheck) name:@"mostReadAvailable" object:nil];
...
-(void)loadNewsCheck
{
    NSLog(@"received @mostReadAvailable");
    [self loadNews];
}
...
- (HomeViewController *)contentViewWithIndex:(int)index
{
    ...
    HomeViewController *page = [storyboard instantiateViewControllerWithIdentifier:CONTENT_IDENTIFIER];
