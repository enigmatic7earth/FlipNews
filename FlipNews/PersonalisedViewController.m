...
#import "PersonalisedViewController.h"
#import <QuartzCore/QuartzCore.h>
#import "PersonalisedContentViewController.h"


#define CONTENT_IDENTIFIER @"PersonalizedContentViewController"
#define FRAME_MARGIN	0 //60
#define PAGE_MIN        0

@interface PersonalisedViewController ()
@property (nonatomic, strong) NSArray *arrNewsData;
@property (nonatomic) Reachability *internetReachability;

-(void)refreshData;
-(void)performNewFetchedDataActionsWithDataArray:(NSArray *)dataArray forFile:(NSString *)fileName;
...
@implementation PersonalisedViewController
...
- (void)viewDidLoad {
    ...
    [appSettings setValue:@"Personalised" forKey:@"currentViewController"]; //_PS addition for checking which fetch method to call in Appdelegate
    [appSettings synchronize];
    //--+--//
    //--Check which News Categories are selected--//
...
-(void) loadNews
{
    ...
    NSString * newsFilePath = [NSString stringWithFormat:@"%@",[docDirectory stringByAppendingPathComponent:@"personalised"]]; // NewsFile
...
- (PersonalisedContentViewController *)contentViewWithIndex:(int)index
{
    ...
    PersonalisedContentViewController *page = [storyboard instantiateViewControllerWithIdentifier:CONTENT_IDENTIFIER];
...
-(void) preparePersonalisedNews
{
    for (NSString * file in _selectedNewsCategories) {
        // append the apt file to "personalized"
        if ([file isEqualToString:@"Arts"]) {
            _newsFileName =@"arts";
        }
        else if ([file isEqualToString:@"Business"])
        {
            _newsFileName = @"business";
        }
        
        NSArray * paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
        NSString * docDirectory = [paths objectAtIndex:0];
        NSString * newsFilePath = [NSString stringWithFormat:@"%@",[docDirectory stringByAppendingPathComponent:@"personalised"]];
        _newsFileName = newsFilePath;
        _personalisedNewsArray = [NSMutableArray arrayWithContentsOfFile:newsFilePath];
        
        NSMutableArray * temp = [NSMutableArray arrayWithContentsOfFile:_newsFileName];
        if (_personalisedNewsArray == nil) {
            _personalisedNewsArray = [NSMutableArray arrayWithArray:temp];
        }
        else
        {
    ...
    if (![_personalisedNewsArray writeToFile:_newsFileName atomically:YES]) {
        NSLog(@"couldn't save %@ file",_newsFileName);
    }
    else
    {
        NSLog(@"saved %@ file", _newsFileName);
    }
}
...
- (IBAction)reloadNews:(UIBarButtonItem *)sender
{
    ...
    if (isInternetConnectionAvailable) {
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
            [self refreshData];
            [self preparePersonalisedNews]; // 15-06-2017
...
-(void)refreshData{
    ...
    for (NSString * newsCategory in _selectedNewsCategories) {
        if ([newsCategory isEqualToString:@"Arts"]) {
            _newsStringURL = ArtsNewsFeed;
            _newsFileName =@"arts";
        }
        else if ([newsCategory isEqualToString:@"Business"])
        {
            _newsStringURL = BusinessNewsFeed;
            _newsFileName =@"business";
            
        }
        
        //After conditions
        XMLParser *xmlParser = [[XMLParser alloc] initWithXMLURLString:_newsStringURL];
        [xmlParser startParsingWithCompletionHandler:^(BOOL success, NSArray *dataArray, NSError *error) {
            
            if (success) {
                [self performNewFetchedDataActionsWithDataArray:dataArray forFile:_newsFileName];
            }
            else{
...
-(void)performNewFetchedDataActionsWithDataArray:(NSArray *)dataArray forFile:(NSString *)fileName{
    ...
    NSArray * paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString * docDirectory = [paths objectAtIndex:0];
    NSString * newsFilePath = [NSString stringWithFormat:@"%@",[docDirectory stringByAppendingPathComponent:fileName]]; // NewsFile
    
    if (![self.arrNewsData writeToFile:newsFilePath atomically:YES]) {
        _newsAvailable = NO;
        NSLog(@"Couldn't save %@ file",fileName);
    }
    else
    {
        _newsAvailable = YES;
